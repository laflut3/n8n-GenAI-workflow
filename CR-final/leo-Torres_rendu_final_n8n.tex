...
\begin{itemize}
  \item Source : table interne \texttt{CITY\_SENSITIVITY} avec des valeurs \textit{Sj} (en \%) par ville.
  \item Calcul : transformation \textit{inverse min-max} pour obtenir un score 0--100 (plus lisible).
  \item Acces via MCP : outil \texttt{find\_city\_score} expose par le serveur MCP.
  \item Correspondance : recherche insensible a la casse (ex. \textit{berlin} = \textit{Berlin}).
  \item Valeurs manquantes : si la ville n'est pas dans la table, l'outil renvoie une erreur (pas de score).
\end{itemize}
...
\]
Le score est arrondi a 2 decimales. Un Sj faible (forte concordance) donne un score plus eleve, tandis qu'un Sj eleve est penalise.
