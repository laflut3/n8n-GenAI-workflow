...
Le filtrage est minimal : pas de dedoublonnage. Un node \textit{Filter} conserve uniquement les offres dont le champ \texttt{salary} n'est pas vide. Les autres offres ne sont pas exportees.
\begin{figure}[H]
  \centering
...
Les offres filtreees sont ecrites dans Google Sheets (equivalent CSV), dans l'onglet \textit{Results true} du tableur \textit{Internships}. Colonnes : company, job\_title, contract\_type, city, country, salary, currency, duration, application\_deadline, skills, languages, source\_url, source\_title, city\_score, summary, remote\_policy.
\begin{figure}[H]
  \centering
...
Un LLM genere une synthese structurée en francais uniquement (sans JSON). Le prompt impose un format en 5 parties : titre, nombre total d'offres, top 3 argumente, statistiques globales (villes frequentes, types de contrat, onsite/hybrid/remote/unknown, salaires si disponibles) et une conclusion courte (2 a 3 phrases), sans invention d'information.
\begin{figure}[H]
  \centering
...
La synthese est envoyee sur Discord via webhook. Un node Code extrait le texte renvoye par le LLM (par ex. \texttt{content.parts[0].text}), tronque a 1900 caracteres si necessaire, et ajoute un marqueur \textit{message tronque} avant envoi.
\begin{figure}[H]
  \centering
...
Role :
Tu es un generateur de rapports synthetiques a partir de donnees structurees.

Contraintes strictes :
- Utilise uniquement les donnees fournies ci-dessous
- N'invente aucune information
- Ne fais aucune supposition
- Si une information est absente ou null, indique-le explicitement ou ignore-la selon le contexte
- La sortie doit etre claire, structuree et directement lisible par un humain
- La reponse finale doit etre uniquement en francais
- Ne retourne pas de JSON, uniquement du texte structure

Tache :
A partir de la liste d'offres de stage fournie, genere un rapport de synthese structure comme suit :

1. Titre
   "Synthese des offres de stage"

2. Nombre total d'offres
   Indique le nombre total d'offres analysees.

3. Top 3 des offres recommandees
   Pour chaque offre :
   - Entreprise
   - Intitule du poste
   - Ville (si disponible)
   - Courte justification basee uniquement sur les donnees presentes
     (exemples autorises : localisation, politique de teletravail, presence d'un salaire, clarte du poste)

4. Statistiques globales
   Presente les statistiques suivantes uniquement si les donnees existent :
   - Villes les plus frequentes
   - Repartition des types de contrat
   - Repartition du mode de travail :
     * onsite
     * hybrid
     * remote
     * unknown
   - Salaires :
     * liste ou fourchette des salaires disponibles
     * precise explicitement si peu ou aucune donnee n'est disponible

5. Conclusion
   Redige une conclusion courte de 2 a 3 phrases maximum resumant :
   - la diversite ou la concentration des offres
   - les tendances principales observees (localisation, teletravail, remuneration)
